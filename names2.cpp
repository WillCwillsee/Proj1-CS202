...
int main ( ) {
    ...
    printNamesToTerminal(tempNames, nameOrder);*/
    for (int m = 0; m < ARR_ROWS; m++){
        for (int n = 0; n < ARR_COLS; n++){
            if (myStringCompare(namesArray, m, namesArray, m+1) < 0){
                /* myStringCopy(tempNames[m][n], namesArray[m][n]);
                 myStringCopy(namesArray[m][n], namesArray[m+1][n]);
                 myStringCopy(namesArray[m+1][n], tempNames[m][n]);
                 tempOrder[n] =*/
